package com.sinosoft.surrender.message.batch.messageutils;
...
public class BackupXmlUtil {
	 ...
	 * @param suffixName
	 */
	public static void backupXML(SurrenderSendMsgUrReqlDTO surrenderSendShortUrlDTO, String xml, String suffixName) {
		OutputStream xmlFileOutputStream = null;
		try {
			// 银行编码
			String BankCode = surrenderSendShortUrlDTO.getBankCode();
			// 交易类型
			String FunFunctionFlag = surrenderSendShortUrlDTO.getFunctionFlag();
			// 获取当前日期
			...
			// 报文名称
			String fileName = surrenderSendShortUrlDTO.getContNo() + "_" + suffixName;
			// 从配置文件中拿到报文的存储路径
